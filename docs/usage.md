# nf-core/uditasflow: Usage

## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/uditasflow/usage](https://nf-co.re/uditasflow/usage)
...
```bash
nextflow run nf-core/uditasflow --input samplesheet.csv --genome GRCh37 -profile docker
```
...
```bash
nextflow pull nf-core/uditasflow
```
...
First, go to the [nf-core/uditasflow releases page](https://github.com/nf-core/uditasflow/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
...
* `docker`
  ...
  * Pulls software from Docker Hub: [`nfcore/uditasflow`](https://hub.docker.com/r/nfcore/uditasflow/)
* `singularity`
